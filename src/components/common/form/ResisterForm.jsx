...
const ResisterForm = () => {
    ...
    const NameDoubleCheck = (name) => {
        doubleCheckNickName(name).then((e) => {
            setDoubleName(true)
            alert("사용가능한 닉네임 입니다.");
        })
            .catch((e) => {
    ...
    return (
        <>
            <Container>
                ...
                <Title fontSize='15px'> 반가워요! 회원가입 후 <Strong>10</Strong>일이 지나면 글작성이 가능해요 :)</Title>

                <InputGroup id='email' type='email' placeholder='전남대학교 이메일을 입력해주세요.' label="이메일"
                ...
                <InputGroup
                    id='username' type='text' placeholder='닉네임을 입력하세요.' label="닉네임"
                ...
                <InputGroup id='password' type='password' placeholder='비밀번호를 입력하세요.' label="비밀번호"
                ...
                <Button
                    onClick={() => {
                        ...
                        //회원가입 요청
                        if (doubleName && doubleEmail && isName && isEmail && isPassword && isPasswordConfirm) {
                            console.log(valueInit.email, valueInit.password, valueInit.username)
                            // register({
                ...
                <Question para='이미 계정이 있으신가요?' children="로그인" onClick={() => { navigate(routes.login); }}></Question>
            </Container >
        </>
